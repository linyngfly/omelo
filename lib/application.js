...
Application.init = function (opts) {
  ...
  if (!!opts.version_key) {
    this.set(Constants.RESERVED.SERVICE_PATH, path.join(Constants.FILEPATH.CONFIG_DIR, 'service', opts.version_key), false);
  }else{
    this.set(Constants.RESERVED.SERVICE_PATH, path.join(Constants.FILEPATH.CONFIG_DIR), 'service', false);
  }
 ...
 * 
 * @param {Object} logger omelo-logger instance without configuration
 *
 * @memberOf Application
 */
...
Application.beforeStopHook = function (fun) {
  logger.warn('this method was deprecated in omelo 0.8');
...
Application.stop = function (force) {
  if (this.state > STATE_STARTED) {
    logger.warn('[omelo application] application is not running now.');
    return;
  }
