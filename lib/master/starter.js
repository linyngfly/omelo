...
starter.run = function(app, server, cb) {
  ...
  if (utils.isLocal(server.host)) {
    ...
    if(process.env.PUB_PLATFORM != null){
      cmd += util.format(' %s=%s ', 'PUB_PLATFORM', process.env.PUB_PLATFORM);
    } 
...
starter.sshrun = function(cmd, host, cb) {
  let args = [];
  args.push(host);
  let ssh_params = omelo.app.get(Constants.RESERVED.SSH_CONFIG_PARAMS);
  if(!!ssh_params && Array.isArray(ssh_params)) {
    args = args.concat(ssh_params);
  }
  args.push(cmd);
  logger.error('Executing ' + cmd + ' on ' + host + ':22', args);
