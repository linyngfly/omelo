...
let Component = function(app, opts) {
  ...
  let originServerPath = path.join(app.getBase(), app.get(Constants.RESERVED.SERVICE_PATH),Constants.FILEPATH.SERVER_PROTOS);
  let presentServerPath = path.join(Constants.FILEPATH.CONFIG_DIR, env, path.basename(Constants.FILEPATH.SERVER_PROTOS));
  let originClientPath = path.join(app.getBase(), app.get(Constants.RESERVED.SERVICE_PATH),Constants.FILEPATH.CLIENT_PROTOS);
  let presentClientPath = path.join(Constants.FILEPATH.CONFIG_DIR, env, path.basename(Constants.FILEPATH.CLIENT_PROTOS));

  this.serverProtosPath = opts.serverProtos || (fs.existsSync(originServerPath) ? originServerPath : presentServerPath);
  this.clientProtosPath = opts.clientProtos || (fs.existsSync(originClientPath) ? originClientPath : presentClientPath);

  this.setProtos(Constants.RESERVED.SERVER, this.serverProtosPath);
  this.setProtos(Constants.RESERVED.CLIENT, this.clientProtosPath);
