...
let Package = require('omelo-protocol').Package;
...
Command.prototype.handle = function(socket, msg) {
  ...
  if(this.useDict) {
    let dictVersion = omelo.app.components.__dictionary__.getVersion();
    if(!msg.sys.dictVersion || msg.sys.dictVersion !== dictVersion){

      // may be deprecated in future
      opts.dict = omelo.app.components.__dictionary__.getDict();

      opts.routeToCode = omelo.app.components.__dictionary__.getDict();
      opts.codeToRoute = omelo.app.components.__dictionary__.getAbbrs();
  ...
  if(this.useProtobuf) {
    let protoVersion = omelo.app.components.__protobuf__.getVersion();
    if(!msg.sys.protoVersion || msg.sys.protoVersion !== protoVersion){
      opts.protos = omelo.app.components.__protobuf__.getProtos();
    }
    opts.useProto = true;
  }

  if(!!omelo.app.components.__decodeIO__protobuf__) {
    if(!!this.useProtobuf) {
      throw new Error('protobuf can not be both used in the same project.');
    }
    let version = omelo.app.components.__decodeIO__protobuf__.getVersion();
    if(!msg.sys.protoVersion || msg.sys.protoVersion < version) {
      opts.protos = omelo.app.components.__decodeIO__protobuf__.getProtos();
    }
    opts.useProto = true;
  }

  if(this.useCrypto) {
    omelo.app.components.__connector__.setPubKey(socket.id, msg.sys.rsa);
  }
