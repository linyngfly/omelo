...
module.exports.loadDefaultComponents = function(app) {
  ...
  if (app.serverType === Constants.RESERVED.MASTER) {
    app.load(omelo.master, app.get('masterConfig'));
  } else {
    app.load(omelo.proxy, app.get('proxyConfig'));
    if (app.getCurServer().port) {
      app.load(omelo.remote, app.get('remoteConfig'));
    }
    if (app.isFrontend()) {
      app.load(omelo.connection, app.get('connectionConfig'));
      app.load(omelo.connector, app.get('connectorConfig'));
      app.load(omelo.session, app.get('sessionConfig'));
      // compatible for schedulerConfig
      if(app.get('schedulerConfig')) {
        app.load(omelo.pushScheduler, app.get('schedulerConfig'));
      } else {
        app.load(omelo.pushScheduler, app.get('pushSchedulerConfig'));
      }
    }
    app.load(omelo.backendSession, app.get('backendSessionConfig'));
    app.load(omelo.channel, app.get('channelConfig'));
    app.load(omelo.server, app.get('serverConfig'));
  }
  app.load(omelo.monitor, app.get('monitorConfig'));
};
...
let loadServers = function(app) {
  app.loadConfigBaseApp(Constants.RESERVED.SERVERS, path.join(app.get(Constants.RESERVED.SERVICE_PATH), Constants.FILEPATH.SERVER));
...
let loadMaster = function(app) {
  app.loadConfigBaseApp(Constants.RESERVED.MASTER, path.join(app.get(Constants.RESERVED.SERVICE_PATH), Constants.FILEPATH.MASTER));
