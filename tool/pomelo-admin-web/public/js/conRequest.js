...
Ext.onReady(function(){
   ...
   let conStore = Ext.create('Ext.data.Store', {
	id:'reqStoreId',
	autoLoad:false,
	pageSize:5,
 ...
 */
let conGrid=Ext.create('Ext.grid.Panel', {
	id:'conGridId',
	region:'center',
    store: conStore,
    columns:[
...
conGrid.addListener('itemdblclick', function(conGrid, rowindex, e){
	...
	let record=conGrid.getSelectionModel().getSelection();
	if(record.length>1){
...
function refresh(){
   ...
   window.parent.client.request('monitorLog', {number:number,logfile:'con-log',serverId:serverId} , function(err, msg) {
    ...
    let store = Ext.getCmp('conGridId').getStore();
    store.loadData(data);
  });
}	
